...
# Resolved by mt_paths (process env OR .env) so standalone runs honor .env.
...
def resolve_vehicle_path(entry):
    ...
    The class name comes from the PACKAGE, never from vehicle_key. A
    placeholder is named after the DataTable row, and a row name is not
    always its asset's name -- row `Trailer_30ft_Log_01` lives in
    `.../Trailer_9m_Flat_01/Trailer_9m_Log_01`, so keying the class off the
    placeholder produced `Trailer_30ft_Log_01_C`, a class that exists in no
    package. The spawner then referenced nothing and silently spawned
    nothing. Same for Police_01, Nuke_Police, Nuke_Taxi and
    PoliceInterceptor_01. vehicle_key stays, but only as the label.
    """
    ...
    veh_key = entry.get("vehicle_key", asset)
    return pkg, f"{asset}_C", veh_key
...
def mesh_source(mods):
    """Resolve the static-mesh entry source as (count, iter_factory).

    iter_factory() yields mesh entries and may be called more than once
    (each call re-reads from scratch) — convert2 streams the entries twice:
    once to build the import cache, once to emit exports.

    Prefers the streamed JSONL sidecar shards that import_meshes writes
    (recorded as the `_imported_shards` marker under mods['static_meshes']),
    so a multi-million-entry export is never materialized in memory. Falls
    back to legacy inline groups for small/hand-authored configs.
    """
    sm = mods.get("static_meshes", {})
    marker = sm.get("_imported_shards") if isinstance(sm, dict) else None
    if marker:
        from mesh_shards import iter_entries, count_entries
        d = marker["dir"]
        pfx = marker.get("prefix")
        cnt = marker.get("count")
        if cnt is None:
            cnt = count_entries(d, pfx)
        return cnt, (lambda: iter_entries(d, pfx))
    legacy = gather_list(mods, "static_meshes")
    return len(legacy), (lambda: iter(legacy))


def _stream_write_asset(asset, exports_list, mesh_ctx, output_path):
    """Serialize `asset` to JSON, streaming the (potentially enormous)
    Exports array instead of building it in memory. Every top-level key is
    written normally except "Exports", which is emitted as: the in-memory
    exports (originals + dealer/BP injections) followed by the generated
    static-mesh actor/component export pair for each streamed mesh entry.

    This is what keeps convert2 bounded for a 6M-mesh export: the 12.4M
    generated export objects are created one pair at a time and discarded,
    never held in a list. UAssetGUI parses the resulting JSON regardless of
    (the lack of) indentation.
    """
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        for key, value in asset.items():
            ...
            f.write(":")
            if key == "Exports":
                _write_exports_streamed(f, exports_list, mesh_ctx)
            else:
                json.dump(value, f, ensure_ascii=False)
        f.write("}")


def _write_exports_streamed(f, exports_list, mesh_ctx):
    f.write("[")
    wrote = False
    ...
    if mesh_ctx:
        base = mesh_ctx["base"]
        cache = mesh_ctx["mesh_cache"]
        level_num = mesh_ctx["level_num"]
        sma_class = mesh_ctx["sma_class"]
        default_sma = mesh_ctx["default_sma"]
        smc_class = mesh_ctx["smc_class"]
        smc0_template = mesh_ctx["smc0_template"]
        i = 0
        for entry in mesh_ctx["factory"]():
            pkg_path, export_name = resolve_mesh_path(entry)
            mesh_imp = cache[pkg_path]
            x = float(entry.get("X", 0)); y = float(entry.get("Y", 0)); z = float(entry.get("Z", 0))
            pitch = float(entry.get("Pitch", 0)); yaw = float(entry.get("Yaw", 0)); roll = float(entry.get("Roll", 0))
            sx = float(entry.get("ScaleX", 1.0)); sy = float(entry.get("ScaleY", 1.0)); sz = float(entry.get("ScaleZ", 1.0))
            actor_num = base + 2 * i + 1
            comp_num = base + 2 * i + 2
            actor_exp = make_raw_export(
            ...
            if wrote:
                f.write(",")
            json.dump(actor_exp, f, ensure_ascii=False)
            f.write(",")
            json.dump(comp_exp, f, ensure_ascii=False)
            wrote = True
            i += 1
    f.write("]")
...
def main():
    ...
    # MTBPInjector (see build.bat step [4b/5]). convert2 must not touch them
    ...
    # STATIC MESHES
    # ======================================================================
    mesh_count, mesh_iter_factory = mesh_source(mods)
    mesh_ctx = None
    if mesh_count:
        for n in (
        ...
        # Pass 1 (streamed): build the mesh import cache + copy missing assets
        # to the mod pak. Deduped by package path, so this touches each unique
        # mesh once regardless of how many instances reference it.
        mesh_cache = {}
        for entry in mesh_iter_factory():
            pkg_path, export_name = resolve_mesh_path(entry)
        ...
        # Reserve 2 export slots per mesh (actor + component), appended AFTER
        # every existing export (originals + dealer/BP injections). The actual
        # export objects are NOT built here — they're streamed straight to the
        # output file in pass 2 (_write_exports_streamed) so 12.4M dicts never
        # coexist in memory. We only record the deterministic actor indices +
        # the dependency-map padding the bookkeeping needs up front.
        base = len(exports)
        print(f"Injecting {mesh_count} static mesh actors (streamed) ...")
        all_new_actor_nums.extend(base + 2 * i + 1 for i in range(mesh_count))
        if depends_map is not None:
            # All-new exports have empty depends entries; share one [] object
            # so the padding costs ~pointer-per-entry, not a list-per-entry.
            depends_map.extend([[]] * (2 * mesh_count))

        mesh_ctx = {
            "base": base,
            "factory": mesh_iter_factory,
            "mesh_cache": mesh_cache,
            "level_num": level_num,
            "sma_class": sma_class,
            "default_sma": default_sma,
            "smc_class": smc_class,
            "smc0_template": smc0_template,
        }
    ...
    # ---- Bookkeeping ------------------------------------------------------
    # exports holds originals + dealer/BP injections; the 2*mesh_count static
    # mesh exports are streamed at write time, so account for them here.
    if asset.get("Generations"):
        asset["Generations"][0]["ExportCount"] = len(exports) + 2 * mesh_count
        asset["Generations"][0]["NameCount"] = len(name_map)

    _stream_write_asset(asset, exports, mesh_ctx, output_path)

    n_dealers = len(dealer_spawns) if dealer_spawns else 0
    n_meshes = mesh_count
    print(f"Done!  {output_path}")
    print(f"  {n_dealers} dealers + {n_meshes} meshes  |  {len(exports) + 2 * mesh_count} exports  |  {len(imports)} imports  |  {len(name_map)} names")
