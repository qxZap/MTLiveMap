...
rem PAKDIR comes from MTMI_GAME_PAKDIR (set by fulltest.bat or your shell).
rem Falls back to a Steam default if you're running modp.bat standalone — but
rem prefer setting MTMI_GAME_PAKDIR once at the top of fulltest.bat so every
...
if not exist "%PAKDIR%" (
    ...
    echo        Set MTMI_GAME_PAKDIR to your game's Paks folder. Browse the
