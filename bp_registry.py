...
from mt_paths import GAME_CONTENT, CELLS_DIR, JEJU_MAIN, MOD_CONTENT_ROOT


# asset_key -> template definition
REGISTRY: dict[str, dict] = {

    # Bus stops, placed by editor coordinates along the first bridge
    # so they can be tested without an editor round trip. A placed stop carries
    # a BusStopGuid and a BusStopDisplayName; the guid needs no special handling
    # because the clone path already regenerates any property whose name ends in
    # 'Guid', deterministically, which is what keeps two stops from sharing one.
    "BusStop": {
        "bp_path":          "/Game/Objects/Mission/Bus/BusStop_01",
        "bp_class":         "BusStop_01_C",
        "source_umap":      JEJU_MAIN,
        "source_actor":     "BusStop_01_C_0",
    ...
    "GasStation": {
        ...
        # FuelPump goes through WP-cell injection (the path parking/garage
        # use successfully). It was previously force-routed into the
        # persistent level because, without the full game tree extracted,
        # FuelPump2 cloned as an unparseable RawExport whose raw-byte
        # name-map indices broke in a different package. Persistent
        # injection avoided that but the actor never actually spawned
        # (confirmed in-game: cell actors appear, persistent ones don't).
        # With full extraction + bundled mappings, FuelPump2 now parses as
        # a proper NormalExport, so DeepCloneProp's FName rebase + prop-ref
        # remap make it safe to clone into a WP cell like any other actor.
    },
    "ParkingLarge": {
...
_TEMPLATES: dict[str, tuple[str, str]] = {
    ...
    # The template decides the map ICON and the point's built-in behaviour:
    # MissionPointType is at default on every vanilla CDO, so the icon follows
    # the CLASS. Cloning the class that matches what the point actually does
    # gets the right icon and the right semantics in one move.
    "farm":               ("Farm_Corn_C",           "CornFarm_2"),
    # NOT gasstation: GasStation_C carries MTFuelPump, which does not survive
    # the class rewrite ("No valid property MTFuelPump in class") and kills the
    # build. Use "liquid" for fuel points and place a GasStation placeholder
    # next to them for the actual pump.
    "warehouse":          ("Warehouse_C",           "Warehouse_Ranch"),
    "supermarket":        ("Supermarket_C",         "Supermarket_C_UAID_107C61471EBE5C2C02_1166770344"),
    "logsupply":          ("LogSupply_C",           "LogSupply_2"),
    "mine":               ("Mine_CopperOre_C",      "Mine_CopperOre_C_UAID_107C61471EBE2EB502_1601273846"),
    "refinery":           ("CrudeOil_Refinery_Input_C", "CrudeOil_Refinery_Input_C_UAID_005056C0000160DD01_1974718923"),
    # NOT militarybase: MilitaryBaseEntrance has 13 children and one of them
    # ("Box") fails the class rewrite at write time.
    "construction":       ("ConstructionSite_C",    "ConstructionSite2"),
    "fastfood":           ("FastFood_Storage_C",    "FastFood_Storage2_11"),
    "liquid":             ("LiquidSupplier_C",      "LiquidSupplier_2"),
}
...
def _load_deprecated() -> set[str]:
    """Cargo rows the game itself marks bDepcreated. They still work today,
    but building an economy on one invites it to vanish in a game update, so
    say so at build time rather than after the fact."""
    p = _Path("CargoImport/cargos/catalog.json")
    if not p.exists():
        return set()
    try:
        return {r["Name"] for r in _json.loads(p.read_text(encoding="utf-8"))
                if r.get("bDepcreated")}
    except Exception:
        return set()


_DEPRECATED_CARGO = _load_deprecated()

# delivery_points.json `new_cargos` adds rows to Cargos_01.uasset; their
# new_id values must be accepted by recipe validation. Populated lazily
# at module-load time after the JSON is parsed (see _load_delivery_points).
_NEW_CARGO_IDS: set[str] = set()
...
def _derive_actor_label(key: str, max_len: int = 40) -> str:
    """Convert 'My_Cool_Delivery_Point' -> 'My Cool Delivery Point'.

    With a current .usmap the actor parses as NormalExport and the label is
    written as a fresh CultureInvariant TextProperty on PointName, so any
    length works. The 14-byte budget only binds on the RawExport fallback
    (stale/missing mappings), where the injector byte-patches inside the
    source label 'NamwonCornFarm' and warns + skips anything longer."""
    label = key.replace("_", " ")
...
def _expand_dp_entry(key: str, cfg: dict) -> dict | None:
    ...
    entry = {
        ...
        # A delivery point is cloned for its ICON and its behaviour, never for
        # the vanilla scenery its class happens to carry (LiquidSupplier_C
        # brings SM_Bld_Silo_Small_01, which put tanks at every gas station).
        # This is specific to delivery points: a fuel pump, garage or parking
        # space is cloned precisely so its mesh shows up, so the flag is set
        # here rather than being a default in the injector.
        "hide_template_mesh": not cfg.get("keep_template_mesh"),
    }
    recipes = cfg.get("recipes") or cfg.get("production_recipes")
    if recipes:
        entry["production_recipes"] = _validate_recipes(key, recipes)
    # Per-point vertical nudge, in uu. Each delivery point has its own
    # placeholder mesh and those do not share a pivot height, so identical
    # editor placement lands some actors buried and others floating. Tune
    # here rather than re-authoring meshes.
    if cfg.get("z_offset"):
        entry["z_offset"] = float(cfg["z_offset"])
    ...
    # Optional explicit WORLD placement: "world": [X, Y, Z, Yaw?]. Lets a DP
    # be dropped at coordinates read off the running game, with no editor
    # placeholder. import_meshes places these verbatim — the import offset is
    # NOT applied, because the numbers are already world-space.
    if "world" in cfg:
        entry["world"] = cfg["world"]
    return entry
...
def _validate_recipes(key: str, recipes: list) -> list:
    ...
    for i, r in enumerate(recipes):
        ...
        for field in ("inputs", "outputs"):
            ...
            # Mod-shipped cargos from `new_cargos` join the valid set so
            # recipes can reference them by new_id.
            bad = [
                n for n in m
                if n not in _VALID_CARGO_NAMES
                and n not in _NEW_CARGO_IDS
            ]
            for n in bad:
                print(f"  [delivery_points] {key} recipe[{i}].{field}: unknown cargo '{n}' — dropped", file=_sys.stderr)
                m.pop(n)
            for n in m:
                if n in _DEPRECATED_CARGO:
                    print(f"  [delivery_points] {key} recipe[{i}].{field}: '{n}' is marked "
                          f"deprecated by the game — still works, but pick a live row",
                          file=_sys.stderr)
