...
from mt_paths import GAME_CONTENT, MAPPINGS, MAPPINGS_TAG, UASSETGUI as _UASSETGUI  # validates env vars at import
MAPPINGS  = str(MAPPINGS)
# Prefer the vendored tools/UAssetGUI.exe; fall back to PATH if missing.
UASSETGUI = str(_UASSETGUI) if _UASSETGUI.is_file() else "UAssetGUI.exe"
...
def to_json(uasset: Path, dst: Path) -> bool:
    ...
    proc = subprocess.Popen(
        # Full .usmap path (not the bare tag) so the vendored UAssetGUI
        # resolves mappings without any external config — see build.bat.
        [UASSETGUI, "tojson", str(uasset), str(dst), "VER_UE5_5", MAPPINGS],
...
def extract_cargo_catalog(cargos_json: Path) -> dict:
    ...
    # The DataTable is not reliably export 0 — which export carries it
    # shifts between game versions. Find it instead of indexing blind.
    table = next((e["Table"] for e in d.get("Exports", []) if "Table" in e), None)
    if table is None:
        raise KeyError(f"no DataTable export in {cargos_json} "
                       f"({len(d.get('Exports', []))} exports)")
    rows = table["Data"]
    catalog = []
    types_seen = set()
    for r in rows:
        ...
        # WeightRange is a Vector2D (min, max) tonnage. Pricing by weight
        # needs it, so pull both ends out of the struct rather than the
        # struct itself, which downstream JSON consumers can't use.
        wmin = wmax = None
        wr = fields.get("WeightRange")
        if wr:
            for sub in wr.get("Value", []) or []:
                if sub.get("Name") == "X": wmin = prop_value(sub)
                elif sub.get("Name") == "Y": wmax = prop_value(sub)
        catalog.append({
            "Name":              name,
            "CargoType":         str(ctype),
            "WeightMin":         wmin,
