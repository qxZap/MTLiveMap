...
- [X] more garages — WHERE, and how many?
- [X] parking spaces — WHERE, and how many?
- [X] death road — where does it start and end?
- [X] marcaje unde e ce (signage) — which places need naming?
...
- [X] **POIs — homes and workplaces.** `Home_<Name>` and `Work_<Name>`.
...
- [X] **Rotate bus stops where they look off.** A stop takes its facing from the
  mesh's Yaw, so turning the marker in the editor turns the shelter. Several
  currently sit at an angle to the road they serve.
- [X] **Zone borders**, if Arini's square should become a real shape.
...
- [X] **Load-time memory: WP LoadingRange was 3x vanilla.** Reported as a
...
- [X] Foliage is OFF in the current pak, DELIBERATELY -- `--skip-foliage` is a
...
- [X] Foliage pop-in on approach.
- [X] Snow should sink you the way mud does. IN PROGRESS.
...
- [X] Console variables never reach the game; `merge_config.py` is inert.
- [X] **Number signs are invisible.** SOLVED as far as the pipeline goes: the
...
- [X] Arini's zone: ONE rectangle now, not 12 tiles. VERIFY IN GAME.
...
- [X] `Zone Test Gangjung` is a leftover diagnostic stop and can be deleted.
- [X] Bridge stop 5 sits east of the boundary, so it belongs to Hallim rather
  than Arini.
