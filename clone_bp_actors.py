...
# Our own StringTable package. NOT the vanilla "Cargo" name: shipping a
...
def build_label_stringtable(entries) -> str | None:
    ...
    same wall the custom cargo rows hit, where they rendered blank. A bus stop
    shows "<MISSING STRING TABLE ENTRY>" instead. So the labels go in a table
    and the stops point at it.

    Separate package path, never the vanilla BusStop table: shipping a
    modified vanilla StringTable crashes MT on world load. Cloned from
    whatever the game ACTUALLY loads, so a mod that overrides the vanilla
    table does not leave ours based on a stale shell.
    """
    labels = {}
    for e in entries:
    ...
    if not labels:
        for q in (out, out.with_suffix(".uexp")):
            if q.exists():
                q.unlink()
                print(f"  cleaned stale {q.name}")
        return None
    src = effective_asset("DataAsset/StringTables/BusStop.uasset")
    if not src.is_file():
        print(f"  bus StringTable source missing at {src} — stop names will not "
              f"render", file=sys.stderr)
        return None
    st_dir.mkdir(parents=True, exist_ok=True)
    import tempfile as _tf
    with _tf.NamedTemporaryFile("w", suffix=".json", delete=False, encoding="utf-8") as tf:
        json.dump(labels, tf); ent = tf.name
    try:
        r = subprocess.run([
...
def materialize_new_cargos(new_cargos: list[dict]) -> bool:
    ...
    stale = [
        ...
        MOD_CONTENT_ROOT / "DataAsset" / "StringTables" / f"{MOD_STRINGTABLE}.uasset",
        MOD_CONTENT_ROOT / "DataAsset" / "StringTables" / f"{MOD_STRINGTABLE}.uexp",
    ]
    if not new_cargos:
    ...
    # Build a mod-owned StringTable so each custom cargo shows its OWN name.
    # Borrowing display_source made Water read "Fuel" and Pezzi "Small Box";
    # a modified vanilla Cargo StringTable crashes on world load, so ours
    # lives at a separate package path and the rows point at it.
    st_dir = MOD_CONTENT_ROOT / "DataAsset" / "StringTables"
    st_dir.mkdir(parents=True, exist_ok=True)
    st_out = st_dir / f"{MOD_STRINGTABLE}.uasset"
    # Clone whatever the GAME actually loads for the cargo StringTable, not
    # the vanilla file on disk. Mods ship their own tables (MajasWheelWorks
    # does exactly this) and one could override the vanilla Cargo table; our
    # ModCargo would then be based on a stale shell.
    vanilla_st = effective_asset("DataAsset/StringTables/Cargo.uasset")
    display = {c["new_id"]: (c.get("display_name") or c["new_id"]) for c in new_cargos}
    if vanilla_st.is_file():
        import tempfile as _tf
        with _tf.NamedTemporaryFile("w", suffix=".json", delete=False, encoding="utf-8") as tf:
            json.dump(display, tf); ent = tf.name
        try:
            r = subprocess.run([
                str(INJECTOR), "make-stringtable",
                "--src", str(vanilla_st), "--output", str(st_out),
                "--namespace", MOD_STRINGTABLE, "--entries", ent,
                "--mappings", MAPPINGS,
            ], capture_output=True, text=True)
            for line in r.stdout.splitlines():
                if line.strip(): print(f"  {line}")
        ...
        print(f"  vanilla StringTable missing at {vanilla_st} — custom cargo will "
              f"borrow display_source labels", file=sys.stderr)
        st_out = None
    ...
    spec = [
        {k: v for k, v in c.items() if k not in ("safety_dps", "display_name")}
        for c in new_cargos
    ]
...
def cell_spec(new_cell: str, x: float, y: float, mod_gen_dir: Path,
    ...
    # coordinate calculation, so raising it collapses distinct cells onto the
...
def main():
    ...
    if os.environ.get("MTMI_SKIP_CARGO") == "1":
        # --skip-cargo: ignore all cargo. Still wipe any stale mod-shipped
        # cargo asset from a prior run (materialize([]) ships none), but build
        # no new Cargos_01 and run no safety-DP injection.
        print("  [--skip-cargo] ignoring cargo (new_cargos + safety_dps)")
        if not materialize_new_cargos([]): return 1
    else:
    ...
    # First-touch lazy bulk-pull: cell resolution loads vanilla cell files
    # MTBPInjector walks. The list of needed cells is runtime-determined
    # from the user's placement set, so we can't pre-extract just the
    # right subset. We pull the full tree (map_cells_all) when it's either
    # SPARSE (never pulled) or STALE (game pak changed since it was pulled,
    # per bootstrap_extract's fingerprint). Both detected here; the
    # repak+AES path means only the game install is needed. Once current,
    # this is a cheap fingerprint check that skips the 2.7GB extraction.
    cells_sparse = CELLS_DIR.is_dir() and len(list(CELLS_DIR.glob("*.umap"))) < 100
    cells_stale = False
    try:
        from bootstrap_extract import stale_bundles
        cells_stale = bool(stale_bundles(["map_cells_all"]))
    except Exception:
        pass
    if cells_sparse or cells_stale:
        why = "sparse" if cells_sparse else "stale (game pak changed)"
        print(f"  [cells] local _Generated_ is {why}; bulk-extracting all WP cells from the game pak (one-time, ~2.7GB)")
        r = subprocess.run([sys.executable, str(Path(__file__).parent / "bootstrap_extract.py"),
                            "map_cells_all"], capture_output=True, text=True)
        if r.returncode != 0:
            print(r.stdout); print(r.stderr, file=sys.stderr); return 1
        for line in r.stdout.splitlines():
            if line.strip(): print(f"    {line}")
    ...
    for i, e in enumerate(entries):
        ...
        if cell is None:
            ...
            return 1
        # Seed cell from vanilla once (for existing vanilla cells). The
        # bulk map_cells_all extraction above guarantees the cell is
        # present in CELLS_DIR, so this is a plain copy into the mod's
        # _Generated_ working dir.
    ...
    for cell, items in grouped.items():
        ...
        for (e, tpl, is_created, assigned_slot) in items:
            ...
            if tpl.get("hide_template_mesh"):
                spec["hide_template_mesh"] = True
            if tpl.get("output_storage_cap") is not None:
                spec["output_storage_cap"] = tpl["output_storage_cap"]
    ...
    if pending_cells or jobs:
        ...
        # Widen WP's streaming radius if asked. Vanilla is 25600 (256 m),
        # which is far too short once every cell carries foliage.
        _lr = os.environ.get("MTMI_WP_LOADING_RANGE", "").strip()
        if _lr:
            try:
                combined["loading-range"] = float(_lr)
            except ValueError:
                print(f"  MTMI_WP_LOADING_RANGE={_lr!r} is not a number — ignored",
                      file=sys.stderr)
        with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False, encoding="utf-8") as tf:
