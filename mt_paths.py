"""
Centralized path resolution for the MTLiveMap pipeline.
...
Required environment variables (set in fulltest.bat or your shell):
  ...
  MTLM_GAME_CONTENT     Folder containing the EXTRACTED vanilla
  ...
  MTLM_MAPPINGS         Path to the .usmap mappings file matching the
  ...
  MTLM_MAPPINGS_TAG     Engine tag UAssetGUI uses with the .usmap
                        (e.g. 'MotorTown718P1'). Must match the
                        filename of the .usmap (without extension).

  MTLM_GAME_PAKDIR      The game's Paks/ folder where the deployed mod
...
_REQUIRED = {
    ...
    "MTLM_MAPPINGS_TAG": {
        ...
        "how": "Same string as the basename of MTLM_MAPPINGS without the '.usmap' suffix.",
    },
    "MTLM_GAME_PAKDIR": {
        "kind": "dir",
...
def _die(missing: list[tuple[str, str]]) -> None:
    ...
    bar = "=" * 72
    sys.stderr.write(f"\n{bar}\n")
    sys.stderr.write("MTLiveMap pipeline cannot start — required paths are missing.\n")
    ...
    sys.stderr.write(
        ...
        "'set \"MTLM_*=...\"' block), or export them in your shell before running\n"
...
GAME_CONTENT: Path = _RESOLVED["MTLM_GAME_CONTENT"]      # type: ignore[assignment]
MAPPINGS: Path     = _RESOLVED["MTLM_MAPPINGS"]          # type: ignore[assignment]
MAPPINGS_TAG: str  = _RESOLVED["MTLM_MAPPINGS_TAG"]      # type: ignore[assignment]
GAME_PAKDIR: Path  = _RESOLVED["MTLM_GAME_PAKDIR"]       # type: ignore[assignment]
